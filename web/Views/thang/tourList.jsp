...
<body>
    ...
    <tbody>
      <c:forEach var="t" items="${tours}">
        <tr>
          ...
          <td>
            <button class="action-btn" onclick="location.href='${pageContext.request.contextPath}/editTour_thang?id=${t.id}'">
              Update
            </button>
            <button class="action-btn" onclick="confirmDelete(${t.id})">
              Delete
            </button>
          </td>
        </tr>
  ...
  const rowsPerPage = 10;
    ...
    function confirmDelete(id) {
      if (confirm('Are you sure to delete tour #' + id + '?')) {
        // Tạo form POST ẩn để xóa
      ...
      if (filter) {
        allRows = allRows.filter(row =>
          row.cells[1].innerText.toLowerCase().includes(filter)
        );
      }

      // Sort trên tập đã filter
      if (sortField) {
        const idxMap = {id:0, name:1, price:2, maxPeoplePerBooking:3};
        const idx = idxMap[sortField];
        allRows.sort((a,b) => {
          let va = a.cells[idx].innerText, vb = b.cells[idx].innerText;
          if (['id','price','maxPeoplePerBooking'].includes(sortField)) {
