 ...
 * @author thang
 */
public class userList_thang extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet userList_thang</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet userList_thang at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        try (Connection conn = new DBContext().getConnection()) {
            ...
            request.getRequestDispatcher("/Views/thang/userList.jsp")
        ...
        try (Connection conn = new DBContext().getConnection()) {
            ...
            if ("changeStatus".equalsIgnoreCase(action)) {
                ...
                response.sendRedirect(request.getContextPath()
                    + "/userList_thang?msg=" + (ok ? "statusOk" : "statusFail"));
            }
            else if ("detail".equalsIgnoreCase(action)) {
                // Chỉ đơn giản redirect tới servlet detail, hoặc bạn có thể forward luôn
                int id = Integer.parseInt(request.getParameter("id"));
         
    response.sendRedirect(request.getContextPath() + "/userDetail_thang?id=" + id);
            ...
            Logger.getLogger(userList_thang.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
