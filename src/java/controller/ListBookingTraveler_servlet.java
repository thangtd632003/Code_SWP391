 ...
 * @author thang
 */
public class ListBookingTraveler_servlet extends HttpServlet {
 ...
 @Override
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ListBookingTraveler_servlet</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ListBookingTraveler_servlet at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
        ...
        try (Connection conn = new DBContext().getConnection()) {
            ...
            request.getRequestDispatcher("/Views/v1/listBookingTraveler.jsp")
                   .forward(request, response);

        } catch (Exception ex) {
            Logger.getLogger(ListBookingTraveler_servlet.class.getName())
        ...
        switch (action) {
            ...
            case "detail":
                ...
                response.sendRedirect(request.getContextPath()
                        + "/DetailBookingTraveler_servlet?bookingId=" + bookingId);
                break;
                  case "update":
                // Chuyển đến servlet detail với bookingId
              int bId = Integer.parseInt(request.getParameter("id"));
String url = request.getContextPath()
             + "/UpdateBookingTraveler_servlet?bookingId=" + bId;
