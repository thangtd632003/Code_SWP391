 ...
 * @author thang
 */
public class ChangePassword_thang extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ChangePassword_thang</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ChangePassword_thang at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
    ...
    throws ServletException, IOException {
       HttpSession session = request.getSession(false);
        ...
        if (email == null || otp == null) {
            response.sendRedirect(request.getContextPath() + "/ForgotPassword_thang");
            return;
        }

        // Forward sang JSP nhập OTP + mật khẩu mới
        request.getRequestDispatcher("Views/thang/changePassword.jsp").forward(request, response);
    } 
    ...
    throws ServletException, IOException {
       HttpSession session = request.getSession(false);
        ...
        try (Connection conn = new DBContext().getConnection()) {
            userDao_thang dao = new userDao_thang(conn);
            ...
            Logger.getLogger(ChangePassword_thang.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
