 ...
 * @author thang
 */
public class ProfileTravler_thang extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ProfileTravler_thang</title>");  
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ProfileTravler_thang at " + request.getContextPath () + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    } 
 ...
 String upd = request.getParameter("update");
        ...
        if (user != null) {
            ...
            request.getRequestDispatcher("Views/thang/profile.jsp").forward(request, response);
        } else {
...
if (birthDateParam != null && !birthDateParam.isEmpty()) {
    ...
    response.sendRedirect(request.getContextPath() 
        + "/ProfileTravler_thang?update=success");
} else {
    response.sendRedirect(request.getContextPath() 
        + "/ProfileTravler_thang?update=fail");
}
